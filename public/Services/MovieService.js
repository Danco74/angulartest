movieApp.factory('MovieService', function () {
    ...
    var addToCollection = function (movie) {
        for (var movieItem in userCollection){
            if (userCollection[movieItem].title == movie.title)
                return false;
        }
        userCollection.push(movie);
        if (isEnoughBudget) {
            budget.value -= 3;
            return true;
        }
    ...
    var removeFromUserCollection  = function(movie){
        for (var i=0;i<userCollection.length;i++){
            if(movie.title==userCollection[i].title){
